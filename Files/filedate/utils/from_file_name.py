...
#-=-=-=-#
	...
	def set_date(self, cma='cma', modify = _yyyy_):
		"""
		Sets the date based on `self.inpt` file name
		...
		"""
		dt_tm = modify(self.inpt)   #$#;print(f'#$# {dt_tm=}') #$#
		try:
		...
		return FileDate(self.inpt).set(**dt_tm_param) # return str(FileDate.self) if not FileDate.SET_SILENT
